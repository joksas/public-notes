...
We can make all tensor products of two covectors by making the following *definition*: if $\alpha, \beta, \gamma, \delta \in \mathcal{V}^*$, $v, w \in \mathcal{V}$ and $a, b \in \mathbb{R}$, then
\begin{align}
\begin{split}
...
\begin{align}
\begin{split}
  \alpha \otimes \gamma &\equiv T_{\mu \nu} e^\mu \otimes e^\nu \\
  &= T_{00} e^0 \otimes e^0 + T_{01} e^0 \otimes e^1 + \dots + T_{33} e^3 \otimes e^3
\end{split}
\end{align}
